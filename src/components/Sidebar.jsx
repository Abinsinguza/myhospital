...
export default function Sidebar() {
  ...
  return (
    ...
    <nav
      className={` shadow-md h-screen p-2 flex flex-col duration-500 bg-blue-400  // 50% transparent blue-400 (RGB: 96,165,250) text-white ${open ? "w-60" : "w-16"} `} >
      {/* Header */}
      ...
      <ul className="flex-1">
        {menuItems.map((item, index) => {
          return (
            <li
              key={index}
              className="px-3 py-2 my-2 hover:bg-blue-500 rounded-md duration-300 cursor-pointer flex gap-2 items-center relative group"
            >
              <div>{item.icons}</div>
              <p
                className={`${
